...
)
{
    ...
    private ImmutableHashSet<ProjectId> projects = projects?.ToImmutableHashSet() ?? [];
    ...
    {
        if (!this.Has(project))
            throw new Exception("The project does not exist in the origin workspace.");

        if (other.Has(project))
            throw new Exception("The project has already been in the destination workspace.");
